function profile(){ // profile function is created
var xmlhttp = new XMLHttpRequest(); // xmlhttp is a new object of XMLHttpRequest()
	xmlhttp.onreadystatechange = function(){ 
		if (this.readyState == 4 && this.status == 200){ /*this condition checks for network connection state success */
/* creates myObj object from parse text of GET attribute of profile.json from xmlhttp.open method */
var myObj=JSON.parse(this.responseText); 
/* This is a first example of respective HTML element having id name "title" is initialized to json object data values and thus rendered during the server runtime of the html page. The same steps is applied to the following lines of code */
...
document.getElementById("telephone1").innerHTML=myObj.contact[0].telephone;
			...
			xmlhttp.open("GET","./api/profile.json",true);
			xmlhttp.send();
}